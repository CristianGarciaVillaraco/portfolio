...
import ProjectPhasesModal from "./ProjectPhasesModal";
...
function MobileTagFilter({
  ...
  const visible = tags.filter((t) => t.toLowerCase().includes(search.toLowerCase()));
...
export default function Projects({ data }: ProjectsProps) {
  ...
  const [phasesProject, setPhasesProject] = useState<Project | null>(null);
  const [prevActiveTags, setPrevActiveTags] = useState(activeTags);
  if (prevActiveTags !== activeTags) {
    setPrevActiveTags(activeTags);
  ...
  const statusLabel = (status: ProjectStatus): string => {
    switch (status) {
      case "planned":
        return tr.ui.statusPlanned;
      case "in-development":
        return tr.ui.statusInDevelopment ?? tr.ui.inDevelopment;
      case "paused":
        return tr.ui.statusPaused;
      case "released":
        return tr.ui.statusReleased;
      case "cancelled":
        return tr.ui.statusCancelled;
      case "abandoned":
        return tr.ui.statusAbandoned;
      case "completed":
        return tr.ui.statusCompleted;
    }
  };
  ...
  return (
    <>
                  ...
                  >
                    <div className="mb-3">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {project.status && (
                          <span
                            className={`shrink-0 text-xs px-2 py-0.5 rounded-full border ${STATUS_CHIP[project.status]}`}
                          >
                      ...
                      {proj?.tagline && (
                        <p className="text-sky-400 text-sm mt-1.5">{proj.tagline}</p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-3">
                      {project.phases && project.phases.length > 0 && (
                        <button
                          onClick={() => setPhasesProject(project)}
                          className="mr-auto text-xs px-2.5 py-1 rounded-full border border-slate-600 text-slate-300 hover:border-sky-500/50 hover:text-sky-300 transition-colors"
                        >
                          {tr.ui.viewPhases}
                        </button>
                      )}
      ...
      <AnimatePresence>
        {phasesProject && phasesProject.phases && (
          <ProjectPhasesModal
            projectName={phasesProject.name}
            phases={phasesProject.phases}
            translations={tr.projects?.[phasesProject.name]?.phases}
            onClose={() => setPhasesProject(null)}
          />
        )}
      </AnimatePresence>
    </>
