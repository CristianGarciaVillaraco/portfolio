...
const levelBar: Record<string, number> = {
  ...
  Intermedio: 55,
  Intermediate: 55,
  "Lectura técnica (A2)": 55,
  "Technical reading (A2)": 55,
  Básico: 35,
  Basic: 35,
  A2: 22,
};
  ...
  return (
    <section id="education" className="py-20 px-6 bg-slate-800 dot-bg">
      <div className="relative max-w-4xl mx-auto" ref={ref}>
        ...
        >
          {tr.educationItems.map((item, i) => (
            <motion.div
              key={i}
              variants={rowVariant}
              className="bg-slate-900/60 rounded-xl p-6 flex flex-col sm:flex-row sm:items-center gap-4 border border-slate-700"
            >
              ...
              <div>
                <p className="text-white font-semibold text-sm leading-snug">{item.degree}</p>
        ...
        <motion.div
          className="mt-16 bg-slate-900/60 rounded-xl p-6 border border-slate-700"
